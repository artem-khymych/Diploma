...
class ParamTuningDialog(QDialog):
    ...
    def _detect_task_type(self):
        """Визначаємо тип задачі на основі типу моделі"""
    ...
    def initUI(self):
        self.setWindowTitle('Налаштування параметрів моделі')
        ...
        model_group = QGroupBox("Інформація про модель")
        model_layout = QFormLayout()
        model_name = QLabel(self.model.__class__.__name__)
        model_layout.addRow("Тип моделі:", model_name)

        task_type_label = QLabel(self._get_task_type_name())
        model_layout.addRow("Тип задачі:", task_type_label)

        model_group.setLayout(model_layout)
        main_layout.addWidget(model_group)

        # Налаштування пошуку
        search_group = QGroupBox("Налаштування пошуку")
        search_layout = QFormLayout()

        self.search_type = QComboBox()
        self.search_type.addItems(["GridSearchCV", "RandomizedSearchCV"])
        search_layout.addRow("Тип пошуку:", self.search_type)

        self.cv_spinbox = QSpinBox()
        self.cv_spinbox.setMinimum(2)
        self.cv_spinbox.setMaximum(20)
        self.cv_spinbox.setValue(5)
        search_layout.addRow("Кількість фолдів (CV):", self.cv_spinbox)

        # Створюємо віджет для вибору метрики відповідно до типу задачі
        self.metric_tab = QTabWidget()
        self._create_metric_tabs()
        search_layout.addRow("Метрика оцінки:", self.metric_tab)

        self.n_iter = QSpinBox()
        self.n_iter.setMinimum(10)
        self.n_iter.setMaximum(1000)
        self.n_iter.setValue(100)
        search_layout.addRow("Кількість ітерацій (для RandomizedSearchCV):", self.n_iter)

        search_group.setLayout(search_layout)
        main_layout.addWidget(search_group)

        # Параметри моделі
        params_group = QGroupBox("Параметри моделі")
        ...
        for param_name, param_value in self.params.items():
            ...
            if isinstance(param_value, bool):
                ...
                # Строковий параметр
                range_layout.addWidget(QLabel("Варіанти (через кому):"), 0, 0)
                ...
                # Інші типи параметрів (списки, словники тощо)
                range_layout.addWidget(QLabel("Варіанти (Python-синтаксис):"), 0, 0)
        ...
        self.run_button = QPushButton("Запустити пошук")
    ...
    def _get_task_type_name(self):
        """Отримати назву типу задачі для відображення"""
        task_type_names = {
            "classification": "Класифікація",
            "Regression": "Регресія",
            "clustering": "Кластеризація",
            "dimensionality_reduction": "Зменшення розмірності",
            "density_estimation": "Оцінка щільності",
            "anomaly_detection": "Виявлення аномалій"
        }
        ...
        try:
            ...
            if self.search_type.currentText() == "GridSearchCV":
                search = GridSearchCV(
                    ...
                    scoring=scoring,
                    n_jobs=1, #TODO check on amount of precessors
                    verbose=1
                )
...
def show_param_tuning_dialog(model, params, X_train=None, y_train=None):
    """
    Показує діалог для налаштування параметрів моделі машинного навчання

    Args:
        model: Модель sklearn для оптимізації
        params: Словник з поточними параметрами моделі
        X_train: Дані для тренування
        y_train: Цільова змінна для тренування (опціонально для кластеризації та деяких інших задач)

    Returns:
        dict: Словник з оптимальними параметрами або пустий словник, якщо користувач скасував діалог
    """
