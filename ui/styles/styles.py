...
def setup_app_style(app):
    """
    Застосовує єдиний стиль для всього наукового-практичного додатку

    Параметри:
    ----------
    app : QApplication
        Екземпляр QApplication, до якого буде застосовано стилі
    """
    ...
    # Встановлення правильних ID для коректної роботи CSS селекторів
    def traverse_widgets(widget):
        """Рекурсивно встановлює ID для віджетів для коректної роботи CSS"""

        if isinstance(widget, QObject):
            # Встановлення ID для кнопок з визначеними іменами
            if hasattr(widget, 'objectName') and widget.objectName():
                pass  # ID вже встановлений
            elif hasattr(widget, 'text'):
                text = widget.text() if callable(widget.text) else ''

                # Встановлення ID для кнопок за їхніми текстовими назвами
            ...
            # Якщо це QLabel з текстом "Статус експерименту..."
            if hasattr(widget, 'text') and callable(widget.text):
                ...
                # Встановлення property для заголовків
    ...
    # Обхід всіх віджетів у програмі, починаючи з головного вікна
...
def update_status_style(label, status):
    """
    Оновлює стиль статусу експерименту
    ...
    label : QLabel
        Мітка зі статусом
    status : str
        Статус: 'finished', 'not_started', або 'running'
    """
    label.setProperty("status", status)
    label.style().unpolish(label)  # Необхідно для оновлення стилю
    label.style().polish(label)  # Необхідно для оновлення стилю
    label.update()
